...
for i=1:10000
    ...
    if(i>1)     
        %METODO DA SECANTE
        dec=(xend(i)-xend(i-1))/(v0(i)-v0(i-1));    %formula do declive normal
                                                %result=x    e    guess=v0

        v0(i+1)=v0(i)+(B-xend(i))/dec;      %guess(3) = guess(2) + (B-result(2))/m                                
    ...
    yD(j,:)=y(:,1:length(k));   %fodasse para o debug disto xd
    xD(j,:)=x(:,1:length(k));
    v0D(j)=v0(end);
    j=j+1;
end
